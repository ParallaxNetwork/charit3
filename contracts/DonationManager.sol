...
contract DonationManager is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Modifier to restrict function access to users who have staked a minimum amount of ETH.
     * Reverts with a standard require message if the user hasn't staked enough.
     */
    modifier onlyStaker() {
        require(deposits[msg.sender] >= 0.0001 ether, "Min stake 0.02 ETH");
        _;
    }
}
